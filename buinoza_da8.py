...
# Графіки
# 📈 Проста регресія зі змінними, які обирає користувач
st.subheader("📈 Побудова простої регресії між двома змінними")

numeric_cols_for_reg = filtered.select_dtypes(include=np.number).columns.tolist()

reg_x_simple = st.selectbox("Оберіть змінну X для регресії:", numeric_cols_for_reg, key="reg_x_simple")
reg_y_simple = st.selectbox("Оберіть змінну Y для регресії:", numeric_cols_for_reg, key="reg_y_simple")

if reg_x_simple != reg_y_simple:
    reg_df = filtered[[reg_x_simple, reg_y_simple]].dropna()
    
    if len(reg_df) >= 2:
        model = LinearRegression()
        model.fit(reg_df[[reg_x_simple]], reg_df[reg_y_simple])
        y_pred = model.predict(reg_df[[reg_x_simple]])

        fig = go.Figure()
        fig.add_trace(go.Scatter(x=reg_df[reg_x_simple], y=reg_df[reg_y_simple],
                                 mode='markers', name='Дані'))
        fig.add_trace(go.Scatter(x=reg_df[reg_x_simple], y=y_pred,
                                 mode='lines', name='Регресія', line=dict(color='orange')))
        fig.update_layout(title=f"Регресія: {reg_y_simple} ~ {reg_x_simple}",
                          xaxis_title=reg_x_simple, yaxis_title=reg_y_simple)
        st.plotly_chart(fig, use_container_width=True)

        st.success(f"Коефіцієнт детермінації R²: {model.score(reg_df[[reg_x_simple]], reg_df[reg_y_simple]):.3f}")
    else:
